...
Neo::Neo(std::shared_ptr<Model> neo_model, std::string _id,
         std::string _neo_id) {
  this->model = neo_model;
  id = _id;
  neo_ref_id = _neo_id;
}
...
void Neo::SetCloseApproach(nlohmann::json close_approach_json) {
  for (nlohmann::json::iterator it = close_approach_json.begin();
    ...
    this->close_approach.emplace_back(
        std::make_unique<CloseApproach>(CloseApproach(*it)));
  }
}
...
std::unique_ptr<Diameter> &Neo::GetDiameter() { return this->diameter; }
...
std::vector<std::unique_ptr<Neo>> &
Neo::GetNeos(std::vector<std::unique_ptr<Neo>> &neos) {
  return neos;
}

std::vector<std::unique_ptr<Neo>> &
Neo::GetNeosDebugOffline(std::vector<std::unique_ptr<Neo>> &neos,
                         std::shared_ptr<Model> neo_model) {
  std::cout << "Start load Sample Data" << '\n';
  std::ifstream f("data/sample.json");
  nlohmann::json data = nlohmann::json::parse(f);
  return InjestJsonDataOffline(data, neos, neo_model);
}
...
std::vector<std::unique_ptr<Neo>> &
Neo::InjestJsonDataOffline(nlohmann::json data,
  ...
  for (int i = 0; i < json_size; i++) {
    ...
    auto n = std::make_unique<Neo>(neo_model, neo_data["id"],
                                   neo_data["neo_reference_id"]);
    ...
    neos.emplace_back(std::move(n));
  }

  return neos;
}

std::vector<std::shared_ptr<Neo>> &
InjestJsonData(nlohmann::json data, std::vector<std::shared_ptr<Neo>> &neos,
               std::shared_ptr<Model> neo_model) {
  auto links = data["links"];
  auto pages = data["page"];
  auto neos_data = data["near_earth_objects"];
  ...
  for (int i = 0; i < json_size; i++) {
    nlohmann::json neo_data = neos_data[i];
    auto n = std::make_unique<Neo>(
        Neo(neo_model, neo_data["id"], neo_data["neo_reference_id"]));
